...
export const Swap = () => {
  ...
  const formatRateValue = (value: number) =>
    Number.isFinite(value) && value !== 0 ? value.toFixed(6) : "0";
  const [limitRate, setLimitRate] = useState(() =>
    formatRateValue(TOKENS[0].priceUsd / TOKENS[1].priceUsd)
  );
  ...
  const [dcaFrequencyUnit, setDcaFrequencyUnit] =
    useState<DcaFrequencyUnit>("Hour");
  ...
  const [selectedSupplyMarketId, setSelectedSupplyMarketId] =
  ...
  const handleCreateDcaOrder = (
    ...
    if (totalToInvest <= 0 || ordersCount <= 0 || perOrderInvestment <= 0)
      return;
    setBalances((prev) => ({
      ...prev,
    ...
    >
      <div
        className={`flex items-start justify-between ${
          options?.compact ? "mb-3" : "mb-4"
        }`}
      >
        <div>
          <p
            className={`uppercase tracking-widest text-gray-500 ${
              options?.compact ? "text-[0.65rem]" : "text-xs"
            }`}
          >
            {label}
          </p>
              ...
              <div className="flex items-center space-x-2 text-xs text-gray-400">
                ...
                {options?.showBalanceActions && (
                  <>
                    <button
                      onClick={() =>
                        setAmount(((options.balance ?? 0) / 2).toString())
                      }
                    ...
                    <button
                      onClick={() =>
                        setAmount((options.balance ?? 0).toString())
                      }
                      className="text-gray-300 border border-gray-700 rounded-full px-2 py-0.5"
                    >
                      MAX
          ...
          <button
            onClick={() =>
              setOpenSelector((prev) =>
                prev === options?.selectorKey
                  ? null
                  : options?.selectorKey ?? null
              )
            }
          ...
          <div className="flex items-center justify-between text-xs text-gray-400">
            ...
            {selectedSupplyOption.apr !== null && (
              <span className="text-[#00FFA3] font-semibold">
                APR {selectedSupplyOption.apr.toFixed(2)}%
              </span>
            )}
          </div>
  ...
  const selectedSupplyOption =
    supplyOptions.find((option) => option.id === selectedSupplyMarketId) ||
    supplyOptions[0];

  const limitExpiryOptions = ["1 Hour", "1 Day", "3 Days", "7 Days", "30 Days"];

  const cycleExpiry = () => {
    const index = limitExpiryOptions.findIndex(
      (option) => option === limitExpiry
    );
                ...
                >
                  ...
                  <Repeat
                    ...
                    className={`transition-transform ${
                      limitRateInPayToken ? "" : "rotate-180"
                    }`}
                  />
                </button>
              </div>
              ...
              >
                <div className="col-span-3 text-white">
                  <div className="flex items-center space-x-2">
                    <span>
                      {order.amountFrom} {order.fromToken} →
                    </span>
                  </div>
                  <div className="text-gray-400 text-xs">
                    {order.amountTo} {order.toToken}
                  </div>
                </div>
                ...
                <div className="col-span-2 flex items-center space-x-2">
                  ...
                  <button className="px-3 py-1.5 rounded-full border border-gray-700 text-gray-300 text-xs">
                    Cancel
                  </button>
                </div>
              </div>
  ...
  const renderDcaControls = () => {
    ...
    const perOrderInvestment =
      dcaInputMode === "perOrder"
        ? basePayValue
        : ordersCount > 0
        ? basePayValue / ordersCount
        : 0;
    const totalInvestment =
      dcaInputMode === "perOrder" ? basePayValue * ordersCount : basePayValue;
    ...
    const estimatedEnd = hasSchedule
      ? new Date(
          Date.now() +
            DCA_FREQUENCY_TO_MS[dcaFrequencyUnit] *
              frequencyValue *
              Math.max(ordersCount - 1, 0)
        )
      : null;
      ...
      <div className="flex bg-[#141722] border border-gray-700 rounded-full text-xs overflow-hidden">
        {(["total", "perOrder"] as DcaInputMode[]).map((modeKey) => (
          <button
            ...
            className={`px-3 py-1 ${
              dcaInputMode === modeKey
                ? "text-white bg-[#1F2333]"
                : "text-gray-400"
            }`}
          >
          ...
          <div className="space-y-4">
              ...
              <div className="flex items-center justify-between text-sm text-gray-400">
                ...
                <div className="flex items-center space-x-2 text-white">
                  <span>
                    ...
                    {dcaCurrentRate.toLocaleString(undefined, {
            ...
            <div className="bg-[#0F1016] rounded-2xl border border-gray-800 p-4 space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white">
                  {formatTokenAmount(totalInvestment, payToken.symbol)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Sell per order</span>
                <span className="text-white">
                  {formatTokenAmount(perOrderInvestment, payToken.symbol)}
                </span>
              </div>
              <div className="flex justify-between">
      ...
      <div className="bg-[#0F1016] rounded-3xl border border-gray-800 p-6 space-y-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex space-x-6 border-b border-[rgba(195,201,228,0.15)]">
            {(["Swap", "Limit", "DCA"] as SwapMode[]).map((tab) => (
              <button
                key={tab}
                onClick={() => setMode(tab)}
                className={`pb-2 text-lg font-medium tracking-wide ${
                  mode === tab
          ...
          <div className="flex flex-wrap items-center gap-3 justify-end">
            <div className="flex items-center space-x-2">
              <button className="flex items-center px-3 py-1.5 text-xs text-gray-300 border border-gray-800 rounded-full space-x-2">
                <span>{slippage}%</span>
                <Settings size={12} />
              </button>
            </div>
